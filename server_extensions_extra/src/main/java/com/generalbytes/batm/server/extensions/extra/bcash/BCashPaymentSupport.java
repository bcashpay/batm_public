...
public class BCashPaymentSupport extends AbstractRPCPaymentSupport {
    private BCashAddressValidator addressValidator = new BCashAddressValidator();
    ...
    public String getCurrency() {
        return CryptoCurrency.BCASH.getCode();
    }
    ...
    public BigDecimal calculateTxFee(int numberOfInputs, int numberOfOutputs, RPCClient client) {
        ...
        try {
            BigDecimal estimate = new BigDecimal(client.getEstimateFee(2)); //bcashd must be run with -deprecatedrpc=estimatefee
